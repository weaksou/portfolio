...
const descriptionContent = [
  `I can consider myself good at HTML, it was the first language I've ever learned, the first code file that I didn't understand a word of it, but It gave me doubt how does this work? and then I started asking questions. I didn't know to google help back then, and after a lot of research, I started learning how to code at the beginning of 2016, but anyway, it made me what I am today. we cannot deny facts <span class="emojie">😀</span>`,
...
creator.addEventListener("click", ()=>{
   document.querySelector("#externalLinks").click();
})

const creditDropDown = document.querySelectorAll(".info-title-bottom");

for (let index = 0; index < creditDropDown.length; index++) {
  creditDropDown[index].addEventListener("click", openDropDown);
}

function openDropDown(event) {
  const classl = event.target.classList;

  if (classl.contains("info-title-bottom")) {
    this.classList.toggle("dropdownborder");
    this.parentNode.classList.toggle("height80");
    this.querySelector(".feather").classList.toggle("fe-rotate");
  }
}
...
//open chat when click on message button
...
//close chat when click on close button
const closeChat = chat.querySelector("#closeChat");
...
//add some shadows to the world
console.log("slow down event listener");
